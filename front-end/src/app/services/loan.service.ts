...
export class LoanService {
  ...
  constructor(private http: HttpClient) {
    ...
    saveLoan(simpleLoan: SimpleLoan): Observable<Loan>{
         return this.http.post<Loan>('http://localhost:8080/rest/loan/api/addLoan', simpleLoan);
     }
      ...
      closeLoan(simpleLoan: SimpleLoan): Observable<Boolean>{
           return this.http.post<Boolean>('http://localhost:8080/rest/loan/api/closeLoan', simpleLoan);
       }
     ...
     searchLoansByEmail(email: string): Observable<Loan[]>{
         return  this.http.get<Loan[]>('http://localhost:8080/rest/loan/api/customerLoans?email='+email);
     }
     ...
     searchLoansByMaximumDate(maxDate: Date): Observable<Loan[]>{
      ...
      return  this.http.get<Loan[]>('http://localhost:8080/rest/loan/api/maxEndDate?date='+maxDateStr);
  }
    ...
    sendEmail(mail: Mail): Observable<boolean>{
      ...
      return this.http.put<boolean>('http://localhost:8080/rest/customer/api/sendEmailToCustomer', mail);
    }
}
