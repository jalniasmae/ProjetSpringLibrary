...
saveLoan(addLoanForm: NgForm){
    ...
    let book = this.http.get<Book>('http://localhost:8080/rest/book/api/searchByIsbn?isbn='+this.simpleLoan.isbn);
    let customer = this.http.get<Customer>('http://localhost:8080/rest/customer/api/searchByEmail?email='+this.simpleLoan.email);
...
saveNewLoan(simpleLoan: SimpleLoan){
  ...
  */
  closeLoan(loan: Loan){
    ...
    let book = this.http.get<Book>('http://localhost:8080/rest/book/api/searchByIsbn?isbn='+loan.bookDTO.isbn);
    let customer = this.http.get<Customer>('http://localhost:8080/rest/customer/api/searchByEmail?email='+loan.customerDTO.email);
    forkJoin([book, customer]).subscribe(results => {
      if((results[0] && results[0].id) && (results[1] && results[1].id)){
